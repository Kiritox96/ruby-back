class WorldController < ApplicationController
    ...
    def show
        ...
        if params[:name].present?
            list = @all_world.find_by(name:params[:name])
            if list
              render json: list
            else
            ...
            list = @all_world.select{|anime| anime[:name].include?(params[:search])}
            if list
              render json: list
            else
            ...
            if params[:random] == "1"
              render json: @all_world.sample
            elsif params[:random] == "4"
              render json: @all_world.shuffle[0..3]
            else
            ...
            list = @all_world.select{|anime| anime[:generi].include?(params[:genere])}
            if list
              render json: list
            else 
            ...
            if params[:type] == 'evidenza'
              ...
              list = @all_world.select{|anime| evidenza.include?(anime[:name])}
              if list
                render json: list
              else
              ...
              list = @all_world.select{|anime| suggeriti.include?(anime[:name])}
              if list
                render json: list
              else
